...
const OrderCard = ({ key, order }) => {
  ...
  return (
    <div key={key} className="w-[500px] bg-[#262626] p-4 rounded-lg  mb-4">
      <div className="flex items-center h-100px gap-5">
          ...
          <div className="flex flex-col items-end gap-2">
            {order.orderStatus === "Ready" ? (
              <>
                ...
                <>
                  <p className="text-blue-500 bg-[#1e3a5f] px-2 py-1 rounded-lg">
                  <FaCheckDouble className="inline mr-2" />{order.orderStatus}
                  </p>
                  <p className="text-[#ababab] text-sm">
                  <FaCircle className="inline mr-2 text-blue-500" /> Thank you for dining with us! 🍽️
                </p>
                </>
            ) : (
              <>
